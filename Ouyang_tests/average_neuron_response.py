...
if __name__ == "__main__":
    ...
    img_cir=circle_sqr(r=r_,width=width_)
    img_ksqr=border_kaniza_sqr(r=r_,width=width_)
    img_n_ksqr=non_kaniza_sqr(r=r_,width=width_)
    img_ksqr_line=border_kaniza_sqr_with_square(r=r_,width=width_)
    img_Lsqr=line_border_sqr(width=width_)
    img_sqr_1=square_generator_bo(beta=True,orientation=90,gamma=True,size=width_)
    img_sqr_2=square_generator_bo(beta=True,orientation=90,gamma=False,size=width_)
    ...
    output_mode = ['prediction', 'E3'] # a list of output modes. Can be 'prediction', 'E0', 'E1', 'E2', 'E3', 'R0', 'R1', 'R2', 'R3', 'A0', 'A1', 'A2', 'A3', 'Ahat0', 'Ahat1', 'Ahat2', 'Ahat3'. If prediction, output prediction; if others, output the units responses
    agent = Agent()
    ...
    sums=np.zeros(10)
    for i in range(10):
        for id in data['bo_info']['E3']['neuron_id']:
            sums[i]+=output_cir['E3'][0,i,id[0],id[1],id[2]]

    sums2=np.zeros(10)
    for i in range(10):
        for id in data['bo_info']['E3']['neuron_id']:
            sums2[i]+=output_ksqr['E3'][0,i,id[0],id[1],id[2]]

    
    sums3=np.zeros(10)
    for i in range(10):
        for id in data['bo_info']['E3']['neuron_id']:
            sums3[i]+=output_non_ksqr['E3'][0,i,id[0],id[1],id[2]]
    
    sums4=np.zeros(10)
    for i in range(10):
        for id in data['bo_info']['E3']['neuron_id']:
            sums4[i]+=output_ksqr_line['E3'][0,i,id[0],id[1],id[2]]
    
    sums5=np.zeros(10)
    for i in range(10):
        for id in data['bo_info']['E3']['neuron_id']:
            sums5[i]+=output_Lsqr['E3'][0,i,id[0],id[1],id[2]]
    
    sums6=np.zeros(10)
    for i in range(10):
        for id in data['bo_info']['E3']['neuron_id']:
            sums6[i]+=output_sqr['E3'][0,i,id[0],id[1],id[2]]
